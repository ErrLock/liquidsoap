...
class output ~infallible ~on_start ~on_stop ~autostart ~kind source =
  ...
  object (self)
    ...
    method output_start =
      ...
      ignore
        (Sdlvideo.set_video_mode ~w:video_width ~h:video_height ~bpp:32
           [`ANYFORMAT; `DOUBLEBUF]) ;
        ...
        | Some (Sdlevent.KEYDOWN k) ->
            ( match k.Sdlevent.keysym with
              | Sdlkey.KEY_f ->
                  ...
                  let mode =
                    if fullscreen then `FULLSCREEN :: mode else mode
                  in
                  ignore
                    (Sdlvideo.set_video_mode ~w:video_width ~h:video_height
                       ~bpp:32 mode)
              | Sdlkey.KEY_q ->
    ...
    method output_send buf =
      ...
      in
      begin
        match Sdlvideo.surface_bpp surface with 16 ->
            Sdl_utils.to_16 rgb surface
        | 32 -> Sdl_utils.to_32 rgb surface | i ->
            failwith (Printf.sprintf "Unsupported format %dbpp" i)
      end ;
