...
object (self)
  ...
  method private get_decoder =
    ...
    let input =
      FFmpeg.Av.open_input url
    in
    let content_type =
      Ffmpeg_decoder.get_type ~url input
    in
    if not (Frame.type_has_kind content_type kind) then
      failwith
        (Printf.sprintf "url %S cannot product content of type %s"
          url (Frame.string_of_content_type content_type));
    container <- Some input;
    let audio =
      try
        Some (Ffmpeg_decoder.mk_audio_decoder ~put_audio:Generator.put_audio input)
      with FFmpeg.Avutil.Error _ -> None
    in
    let video =
      try
        Some (Ffmpeg_decoder.mk_video_decoder ~put_video:Generator.put_video input)
      with FFmpeg.Avutil.Error _ -> None
    in
    Ffmpeg_decoder.mk_decoder ~set_mode:Generator.set_mode ~add_break:Generator.add_break
                              ~audio ~video ~container:input
...
let () =
  let k = Lang.univ_t () in
    Lang.add_operator "input.ffmpeg" ~active:true
      ~descr:"Decode a url using ffmpeg."
      ~category:Lang.Input
