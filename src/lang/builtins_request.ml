...
let () =
  add_builtin "request.create.raw" ~cat:Liq
    ~descr:
      "Create a raw request, i.e. for files that should not be decoded for \
       streaming. Creation may fail if there is no available RID, which \
...
let () =
  add_builtin "request.uri" ~cat:Liq ~descr:"Initial URI of a reuqest."
    [("", Lang.request_t (Lang.univ_t 1), None, None)]
    Lang.string_t
    (fun p ->
      let r = Lang.to_request (List.assoc "" p) in
