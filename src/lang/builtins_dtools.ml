...
let () =
  let set cast path v =
    try (cast (Configure.conf#path (Dtools.Conf.path_of_string path)))#set v
  ...
  add_builtin ~cat:Liq "set"
    ...
    (fun p ->
      ...
      try
        begin
          match (Lang.assoc "" 2 p).Lang.value with Lang.Tuple [] ->
              set Dtools.Conf.as_unit path ()
          | Lang.String s -> set Dtools.Conf.as_string path s | Lang.Int s ->
              set Dtools.Conf.as_int path s
          | Lang.Bool s -> set Dtools.Conf.as_bool path s | Lang.Float s ->
              set Dtools.Conf.as_float path s
          | Lang.List l ->
              let l = List.map Lang.to_string l in
      ...
      with Dtools.Conf.Mismatch _ ->
        let t = Configure.conf#path (Dtools.Conf.path_of_string path) in
        let kind =
          match t#kind with
            | Some "unit" ->
                "of type unit"
        ...
        in
        let msg =
          Printf.sprintf "key %S is %s, thus cannot be set to %s"
            (Lang.to_string s) kind
