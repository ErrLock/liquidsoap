        ...
        match Request.from_id id with
          ...
          | Some r ->
              if Request.is_on_air r then "ERROR: Request already on air!"
                ...
                try
                  Request.destroy
                    (Rqueue.remove_pred queue (fun _ r ->
                         Request.get_id r = id)) ;
                  "OK"
    ...
    self#register_command "move" ~usage:"move <rid> <pos>"
      ~descr:"Move request <rid> in the secondary queue." (fun a ->
        if Str.string_match move_re a 0 then (
          ...
          try
            let req, _ =
              Rqueue.remove_pred_index queue (fun _ r ->
                  Request.get_id r = rid)
            in
            try
              Rqueue.insert queue pos req ;
              "OK"
            with Rqueue.Not_found ->
              Request.destroy req ; "ERROR: Insertion failed, request lost!"
...
let () =
  Lang.add_operator "request.equeue" ~category:Lang.Input
    ~descr:
      "Receive URIs from users, and play them. Insertion and deletion \
