    ...
    * rid of reload overlaps, which requires more duppy synchronization. *)
    ...
    method get_next_request : Request.t option =
      ...
      let rec get_uri () =
        ...
        if self#check_next r then (Mutex.unlock mylock ; Some r)
        else (
    ...
    method playlist_wake_up =
      ...
      Mutex.lock mylock ;
      begin
        match reload with Never | Watch -> () | Every_N_rounds n ->
            round_c <- n
    ...
    method get_next =
      Tutils.mutexify mylock (fun n ->
          match random with
            | Normal ->
                Array.init n (fun i ->
                    !playlist.((index_played + i + 1)
                               mod Array.length !playlist))
            | Randomize ->
                Array.init
...
class playlist ~kind ~on_track ~mime ~reload ~random ~check_next ~length
  ...
  object (self)
    ...
    inherit
      vplaylist ~on_track ~mime ~reload ~random ~timeout ~prefix uri as pl
    ...
    method wake_up activation = pl#playlist_wake_up ; super#wake_up activation
...
(** Safe playlist, without queue and playing only local files,
  * which never fails. *)
class safe_playlist ~kind ~on_track ~mime ~reload ~random ~prefix
  local_playlist =
  object (self)
    inherit
      vplaylist
    ...
    method wake_up = pl#playlist_wake_up ; super#wake_up
  ...
  let proto =
    [ ( "mode",
      ...
      ( "reload_mode",
        ...
        Some
          "Unit of the reload parameter, either 'rounds', 'seconds' or \
      ...
      ( "on_track",
        ...
        Some
          "Function to execute when playlist is about to play its next track. \
           Receives track position in the playlist and wether this is the \
           last track. Force a reload by returning `true` in this function. "
      );
      ( "prefix",
        Lang.string_t,
        Some (Lang.string ""),
      ...
      if arg = 0 then Never
        ...
        match ss with
          | "rounds" ->
              Every_N_rounds arg
          | "seconds" ->
              Every_N_seconds (float_of_int arg)
          | _ ->
              raise
    ...
    ( "check_next",
      ...
      Some
        "Function used to filter next tracks. A candidate track is only \
         ...
         before resolution, hence metadata will only be available for \
         requests corresponding to local files. This is typically used to \
         avoid repetitions, but be careful: if the function rejects all \
         attempts, the playlist will enter into a consuming loop and stop \
         playing anything." )
  in
